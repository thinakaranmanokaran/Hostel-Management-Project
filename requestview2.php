...
<?php
      ...
      $full=$r['Fname']. $r['Lname']; */
       ...
       $sql=mysqli_query($connect,"select * from leave_apply");
...
<?php
            ...
            $s=mysqli_query($connect, "select * from student where student_roll_no='".$r1['student_roll_no']."'");
      $r=mysqli_fetch_array($s);
      $full=$r['Fname']. $r['Lname'];
		  ...
		  <td>
            ...
            {
                ?>
                                <a href="requestview2.php?act=1&id=<?php echo $r1['id'];?>">Approve</a>
<?php
            }
            elseif($r1['status']==2)
            {
...
extract($_REQUEST);
if($act==1){
    $up=mysqli_query($connect,"update leave_apply set status='2' where id='$id'");
    if($up){
        ?>
        <script>
            alert("Leave Request Approved")
            window.location.href="requestview2.php";
        </script>
        <?php
    }
}
