...
$student_roll_no=$_SESSION['student_roll_no'];
  ...
  if(isset($submit))
  {
    ...
    $rdate_with_time = $rdate . ' ' . date("H:i:s"); 
    
    echo $rdate_with_time;
      
  
  $mq=mysqli_query($connect,"select max(id) from student_monitor");
  $mr=mysqli_fetch_array($mq);
  $id=$mr['max(id)']+1;

  $ins=mysqli_query($connect,"insert into student_monitor(id,student_roll_no,status,reason,rdate) values($id,'$student_roll_no','$status','$reason','$rdate_with_time')");
      if($ins)
      {
          if($status=='Out')
          {
          ...
          $faname=$r2['faname'];
          $mobile1=$r2['mobile1'];
          $mess1="Student entry alert in your mail";
          $mess="Dear [$faname] inform you that your child, [$fullName], has hostel out due to reason is [$reason].
          Date:-$rdate_with_time
          Thanking You";
           

  		         echo '<iframe src="http://iotcloud.co.in/testsms/sms.php?sms=emr&name='.$faname.'&mess='.$mess1.'&mobile='.$mobile1.'"></iframe>'; 

                          echo '<iframe src="http://iotcloud.co.in/testmail/testmail1.php?message='.$mess.'&email='.$email.'&subject=Student Entry In/Out" frameborder="0" style="display:none"></iframe>';
...
<section class="contact py-5">
    <div class="container">
        <center><h2 class="heading text-capitalize mb-sm-5 mb-4">Entry In/Out</h2></center>
        <div class="mail_grid_w3l" style="padding-left:400px;width: 100%;margin: 0 auto;">
            <form action="" method="post">
                <div class="row">
                    <div class="col-md-6 contact_left_grid" data-aos="fade-right">
                        <div class="contact-fields-w3ls">Choose In/Out
                            <select name="status" id="">
                                <option value="">--Choose In/Out--</option>
